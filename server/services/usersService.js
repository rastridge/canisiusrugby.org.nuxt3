...
async function _setPerms(aPerms, id) {
  ...
  await doDBQueryCanisiusrugby(sql)
...
async function _getPerms(id) {
  ...
  const sql = `SELECT
							...
							FROM
								can_admin_perms p, can_admin_apps a, can_admin_users u
							WHERE
  ...
  const perms = await doDBQueryCanisiusrugby(sql)
...
async function authenticate({ username, password }) {
  ...
  const sql = `SELECT *
								FROM can_admin_users
								WHERE deleted = 0`

  const users = await doDBQueryCanisiusrugby(sql)
...
async function getOne(id) {
  ...
  const user = await doDBQueryCanisiusrugby(sql)
...
async function deleteOne(id) {
  ...
  try {
    ...
    let sql = `DELETE FROM can_admin_users
								WHERE admin_user_id= ${id}`
    await conn.execute(sql)

    sql = `DELETE FROM can_admin_perms WHERE admin_user_id = ${id}`
    await conn.execute(sql)
...
async function addOne({ admin_user_name, password, admin_user_email, perms }) {
  ...
  try {
    ...
    if (!user) {
      ...
      // sendEmail(
      // 	'ron.astridge@me.com',
      // 	'Canisius University Rugby Club  Admin Account Modification',
      // 	'An account for user ' +
      // 		lc_admin_user_name +
...
async function changeStatus({ id, status }) {
  const sql =
    `UPDATE can_admin_users SET STATUS = "` +
    status +
    `" WHERE admin_user_id  = ` +
    id
  const user = await doDBQueryCanisiusrugby(sql)
...
async function getApps() {
  ...
  const apps = await doDBQueryCanisiusrugby(sql)
...
async function getAppPerms() {
  const sql = `SELECT
							...
							FROM
								can_admin_apps as a,
								can_admin_perms as p,
								can_admin_users as u
							WHERE
								a.admin_app_id = p.admin_app_id
								AND
...
async function resetRequest({ username }) {
  ...
  const rows = await doDBQueryCanisiusrugby(sql)
...
async function resetPassword({ username, password }) {
  ...
  const sql = `UPDATE can_admin_users
					SET
						admin_user_pass = ?,
						modified_dt= NOW()
					WHERE
