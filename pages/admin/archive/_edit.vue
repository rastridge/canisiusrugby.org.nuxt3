<template>
	<div class="edit">
		<adminheader title="Edit archive" />
		<p
...
export default {
	...
	created() {
		archiveService.getOne(this.id).then((archive) => {
			this.setInfo(archive)
		})
	},
	methods: {
		handleFileUpload() {
		...
		setInfo(u) {
			this.archive_title = u.archive_title
			this.archive_description = u.archive_description
			this.archive_filepath = u.archive_filepath
			this.archive_date = moment(u.archive_date).format('YYYY-MM-DD')
		},
		returnToList() {
