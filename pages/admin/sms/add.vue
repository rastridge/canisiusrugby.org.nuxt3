...
export default {
	...
	methods: {
		...
		addSMS(send) {
			const { sms_member_type_id, sms_subject, sms_body_text } = this
			smsService
				.addOne(sms_member_type_id, sms_subject, sms_body_text)
				.then((sms) => {
					if (!sms.error) {
						this.id = sms.insertId
						if (send === 'sendNow') {
							this.sendSMS()
						}
						this.returnToList()
