...
export default {
	...
	created() {
		this.initDates()
		newsService.getOne(this.id).then((news) => {
	...
	methods: {
		...
		setNewsInfo(u) {
			...
			this.release_dt = u.news_release_dt
			this.expire_dt = u.news_expire_dt
		},
		returnToList() {
 ...
 */
		...
		handleSubmit(e) {
			...
			if (this.$v.$invalid) {
				...
				newsService
					.editOne(id, title, synop, article, event_dt, release_dt, expire_dt)
					.then((news) => {
						if (!news.error) {
							this.returnToList()
						} else {
