<template>
	<div class="add">
		<adminheader title="Admin User Add" />
		<p
		...
		<form class="form">
			...
			<!-- Username input-->
			<div class="form-group">
				<label class="col-sm-6 col-md-8 control-label" for="user"
					>Username:
				</label>
				<div class="col-sm-6 col-md-8">
					<input
			...
			<div class="form-group">
				<label class="col-sm-6 col-md-8 control-label" for="email"
					>Email:
				</label>
				<div class="col-sm-6 col-md-8">
					<input
					...
					/>
					<span v-if="!$v.email.email" class="error">Invalid email</span><br />
	...
	methods: {
		...
		handleSubmit(e) {
			...
			if (this.$v.$invalid) {
				...
				const { user, pass, email } = this

				userService.addOne(user, pass, email).then((user) => {
					if (!user.error) {
						this.returnToList()
					} else {
						this.submitStatus = 'ERROR'
