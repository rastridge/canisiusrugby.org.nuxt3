...
<template>
	<div class="edit">
		<adminheader title="Payments Edit" />
		<p
...
export default {
	...
	created() {
		paymentsService.getOne(this.id).then((content) => {
	...
	methods: {
		...
		setInfo(u) {
			this.payment_title = u.payment_title
			this.payment_description = u.payment_description
			this.payment_paypal_button = u.payment_paypal_button
			this.release_dt = u.release_dt
			this.expire_dt = u.expire_dt
		},
		returnToList() {
